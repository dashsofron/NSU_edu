...
@Component
public class WishListGoodExtractor implements ResultSetExtractor<List<NeededGood>> {
    ...
    public List<NeededGood> extractData(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            String goodID = rs.getString("WISH_LIST_GOOD_ID");
            NeededGood good;
            if (goods.containsKey(goodID))
                good = goods.get(goodID);
            else {
                good = new NeededGood();
                good.setId(rs.getInt("WISH_LIST_GOOD_ID"));
